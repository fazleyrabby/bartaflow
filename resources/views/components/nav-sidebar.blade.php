@php
    $navItems = [
        ...
        ['label' => 'Templates', 'route' => 'templates.index', 'icon' => 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z'],
        ['label' => 'Messages', 'route' => 'messages.index', 'icon' => 'M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z'],
    ...
    $settingsItems = [
        ...
        ['label' => 'Activity log', 'route' => 'settings.activity', 'icon' => 'M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z', 'can' => 'viewAny-activity'],
    ];
@endphp
...
>
    ...
    <div class="flex-1 overflow-y-auto px-3 py-4 space-y-1">
        {{-- Main nav --}}
        <p class="px-3 pb-1 text-[11px] font-bold uppercase tracking-wider text-gray-400">Main</p>
        @foreach ($navItems as $item)
            ...
            <a
                ...
                @class([
                    ...
                    'bg-gray-900 text-white shadow-sm'                          => $active,
                    'text-gray-600 hover:bg-gray-100 hover:text-gray-900'       => ! $active,
                    'cursor-default opacity-40 pointer-events-none'             => ! $item['route'],
                ])
            >
                <svg @class(['h-5 w-5 shrink-0 transition-colors', 'text-white' => $active, 'text-gray-400 group-hover:text-gray-600' => ! $active]) fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        {{-- Settings nav --}}
        <p class="px-3 pb-1 pt-4 text-[11px] font-bold uppercase tracking-wider text-gray-400">Settings</p>
        @foreach ($settingsItems as $item)
            @if (($item['can'] ?? null) === 'viewAny-activity' && ! Gate::allows('viewAny', App\Models\ActivityLog::class))
                @continue
            @endif
            @php
                $href   = $item['route'] ? route($item['route']) : '#';
    ...
    <div class="shrink-0 border-t border-gray-200 p-4">
        <div class="flex items-center gap-3">
            ...
            <form method="POST" action="{{ route('logout') }}" class="shrink-0">
                @csrf
                <button type="submit" title="Sign out" class="rounded p-1 text-gray-400 hover:bg-gray-100 hover:text-red-600 transition-colors">
