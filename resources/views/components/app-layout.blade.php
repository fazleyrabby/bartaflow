...
<body class="bg-gray-50 text-gray-900" x-data="{ sidebarOpen: window.innerWidth >= 1024 }">
    <div class="flex h-screen overflow-hidden">
        ...
        <div class="scroll-gutter-stable relative flex flex-1 flex-col overflow-y-auto overflow-x-hidden">
            ...
            <header class="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-gray-200 bg-white px-4 sm:px-6 lg:px-8">
                <div class="flex items-center gap-4">
                    ...
                    <button
                        ...
                        class="rounded-lg p-2 text-gray-500 hover:bg-gray-100 focus:outline-none lg:hidden"
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div x-data="{ open: false }" class="relative">
                        <button
                            ...
                            class="inline-flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-3 py-1.5 text-sm font-medium text-gray-900 hover:bg-gray-100 focus:outline-none"
                        >
                            ...
                            @foreach ($userWorkspaces as $ws)
                                <li>
                                    <form method="POST" action="{{ route('workspaces.switch') }}">
                                        ...
                                        <button type="submit" class="flex w-full items-center gap-2 rounded-md px-3 py-2 text-left text-sm font-medium hover:bg-gray-100 {{ ($currentWorkspaceId ?? null) === $ws->id ? 'bg-emerald-50 text-emerald-700' : 'text-gray-700' }}">
                    ...
                    <div x-data="{ open: false }" class="relative">
                        <button
                            @click="open = !open"
                            @click.outside="open = false"
                            class="flex h-9 w-9 items-center justify-center rounded-full bg-emerald-100 text-sm font-bold text-emerald-700 hover:bg-emerald-200"
            ...
            @if (isset($workspace) && $workspace->isSuspended())
                <div class="flex items-center border-b border-yellow-200 bg-yellow-50 p-4 text-sm text-yellow-800">
